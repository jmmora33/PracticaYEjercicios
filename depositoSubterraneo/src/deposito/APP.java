	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		if(volumenTotal > volumenDepositos) {
			...
			while (volumenTotal>0 && index < cantidadDeDepositos) {
				if (index +1 < cantidadDeDepositos && depositos.get(index).getAltura() != depositos.get(index+1).getAltura()) {
					...
					while(index +cantDepositosIguales+1 < cantidadDeDepositos && depositos.get(index +cantDepositosIguales).getAltura() == depositos.get(index +cantDepositosIguales+1).getAltura())
					...
					if(index +cantDepositosIguales+1 >= cantidadDeDepositos) {
						...
						alturaRel = depositos.get(index).getAltura() - depositos.get(index+cantDepositosIguales +1).getAltura();
						while(index < cantidadDeDepositos) {
